...
export default class LabelKlickerComponent {
  ...
  #telOp() {
    ...
    document.getElementById("add").addEventListener(clickEvent, () => {
      ...
      this.#triggerEventRandomPunten();
      this.#startInterval();
      this.#spawnBananenOnClick();
    });
  }

    #spawnBananenOnClick(){
      const img = document.createElement("img");
      img.src = "img/banaanOnClick.png";
      img.width = 200;
      img.height = 200;
      img.className = "mini-banaan";
    ...
    // if (this.#labelKlicker.som > randomPunten && this.#labelKlicker.som <= this.#labelKlicker.som + 200){
      if (this.#labelKlicker.som > randomPunten){
        ...
        if (this.#labelKlicker.som >= 1000){
          this.#labelKlicker.som = (this.#labelKlicker.som - this.#labelKlicker.som) - randomMultiplier + this.#labelKlicker.som++;
  ...
  #startInterval() {
    if (this.#labelKlicker.som >= 1000) {
        ...
        // Controleer of de farm al is gemaakt
        if (!this.farmCreated) {
            ...
            // Markeer dat de farm is gemaakt
            this.farmCreated = true;

            // Start het interval voor het genereren van punten voor de farm
            this.#startFarmInterval();
        }
    }
}
...
#startFarmInterval() {
    setInterval(() => {
        document.getElementById("Nummer").textContent = this.#labelKlicker.som - this.#labelKlicker.som + this.#labelKlicker.som++; // Verhoog de som met 1
   }, 3000); // 3000 milliseconden = 3 seconde
  if (this.#labelKlicker.som >= 2000){
    setInterval(() => {
      document.getElementById("Nummer").textContent = this.#labelKlicker.som - this.#labelKlicker.som + this.#labelKlicker.som++; // Verhoog de som met 1
 }, 2200); // 2200 milliseconden = 2.2 seconde
   }
}
...
#stop() {
  ...
  stopButton.onclick = () => {
      ...
      document.getElementById("Nummer").textContent = 0;
  ...
  #toonTotaleScore(){
    let totaleScore = `${this.#labelKlicker.som}`
